            ...
            <div class="row mb-4 border pb-3">
                ...
                <div class="row mt-4">
                    <div class="col-md-6 mb-3">
                        ...
                        <a href="/user/order/{{$order->id}}" class="btn btn-info btn-sm">Xem chi tiết</a>
                        <button class="btn btn-warning btn-sm">Mua Lại</button>
                        @if($order->status == 'pending')
                            <button id="cancelOrderBtn" class="btn btn-danger btn-sm">Hủy đơn hàng</button>
                        @endif
                    </div>
                    <form id="cancelOrderForm" action="/user/order/{{$order->id}}/cancel" method="POST" style="display: none;">
                        @csrf
                    </form>
                </div>
            </div>
